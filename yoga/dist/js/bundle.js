...
function form() {
  ...
  form.addEventListener('submit', function (event) {
    ...
    postData(json).then(function () {
      ...
      return statusMessage.innerHTML = message.faillure;
    }).then(function () {
      return clearInputs();
    });
  });
...
function modal() {
  ...
  function showModal(startHandler, overlay, closeHadndler) {
    ...
    startHandler.addEventListener('click', function () {
      ...
      this.classList.add('more-splash'); // deny body allow scroll
    ...
    closeHadndler.addEventListener('click', function () {
      ...
      startHandler.classList.remove('more-splash'); // allow body scroll 
    ...
    if (target && target.classList.contains('info-header-tab')) {
      for (var i = 0; i < tab.length; i++) {
        if (target == tab[i]) {
          ...
          hideTabContent(0); // show current tab content
...
function timer() {
  // Timer
  var deadline = '2020-01-30';

  function getTimerRamaining(endTime) {
    var t = Date.parse(endTime) - Date.parse(new Date()),
        seconds = Math.floor(t / 1000 % 60),
        minutes = Math.floor(t / 1000 / 60 % 60),
        hours = Math.floor(t / (1000 * 60 * 60)); // days if you need
  ...
  function setClock(id, endTime) {
    ...
    function updateClock() {
      var t = getTimerRamaining(endTime);
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  var tabs = __webpack_require__(/*! ./partials/tabs */ "./src/js/partials/tabs.js"),
